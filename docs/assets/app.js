...
function scoreEntry(entry, query) {
  ...
  haystacks.forEach(([text, weight]) => {
    ...
    if (source.includes(q)) score += weight;
  });
  return score;
}
  ...
  root.innerHTML = `
    <section class="search-panel card">
      <div class="search-grid">
        <input id="search-query" type="search" placeholder="输入问题、概念、人物、主题..." />
      ...
      <p class="meta">站点内提供结构化检索；语义检索通过 OpenAI embeddings + FAISS 离线管线生成，可供 Agent 或本地工具调用。</p>
    </section>
    ...
    results.innerHTML = matched
      .map(({ entry }) => `
        <article class="card search-result">
          ...
          <p>${entry.summary || ""}</p>
          <p class="search-meta">${[entry.section, entry.domain, entry.version, entry.author, entry.book].filter(Boolean).join(" · ")}</p>
          <div class="tag-list">${(entry.tags || []).map((item) => `<span class="tag">${item}</span>`).join("")}</div>
        </article>
      `)
      .join("");
  };
